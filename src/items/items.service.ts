...
export class ItemsService {
  ...
  async get(
    ...
    let data: any = await this.sharedService.getStatistics(
      ...
      endDate,
      aggregate,
      process.env.SOLR_VIEWS_KEY_ITEM,
      process.env.SOLR_DOWNLOADS_KEY_ITEM,
    );
  ...
  async getItems(uuid: string, limit: number, page: number): Promise<any> {
    ...
    const query = this.dataSource
      ...
      .leftJoin(
        'handle',
        'handle',
        `handle.resource_id = item.uuid AND handle.resource_type_id = ${Number(
          process.env.DSPACE_ITEM_RESOURCE_TYPE_ID,
        )}`,
      )
      .leftJoin(
        'metadatavalue',
        'metadatavalue',
        'metadatavalue.dspace_object_id = item.uuid',
      )
      .where('item.in_archive = TRUE AND item.withdrawn = FALSE')
      .groupBy('item.uuid');
