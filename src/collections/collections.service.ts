...
export class CollectionsService {
  ...
  async get(
    ...
    // Get collections
    const collections = await this.getCollections(uuid, limit, page);
    let data: any = await this.sharedService.getStatistics(
      collections,
      startDate,
      endDate,
      aggregate,
      process.env.SOLR_VIEWS_KEY_COLLECTION,
      process.env.SOLR_DOWNLOADS_KEY_COLLECTION,
    );
  ...
  async getCollections(
    ...
    const query = this.dataSource
      ...
      .leftJoin(
        'handle',
        'handle',
        `handle.resource_id = collection.uuid AND handle.resource_type_id = ${Number(
          process.env.DSPACE_COLLECTION_RESOURCE_TYPE_ID,
        )}`,
      )
      .leftJoin(
        'metadatavalue',
        'metadatavalue',
        'metadatavalue.dspace_object_id = collection.uuid',
      )
      .groupBy('collection.uuid');

    if (titleMetadataFieldId) {
      query.andWhere(
        'metadatavalue.metadata_field_id = :titleMetadataFieldId',
        { titleMetadataFieldId },
      );
    }
