...
export class CommunitiesService {
  ...
  async get(
    ...
    // Get communities
    const communities = await this.getCommunities(uuid, limit, page);
    let data: any = await this.sharedService.getStatistics(
      communities,
      startDate,
      endDate,
      aggregate,
      process.env.SOLR_VIEWS_KEY_COMMUNITY,
      process.env.SOLR_DOWNLOADS_KEY_COMMUNITY,
    );
  ...
  async getCommunities(
    ...
    const query = this.dataSource
      ...
      .leftJoin(
        'handle',
        'handle',
        `handle.resource_id = community.uuid AND handle.resource_type_id = ${Number(
          process.env.DSPACE_COMMUNITY_RESOURCE_TYPE_ID,
        )}`,
      )
      .leftJoin(
        'metadatavalue',
        'metadatavalue',
        'metadatavalue.dspace_object_id = community.uuid',
      )
